    ...
    if (Object.keys(array == 'select')) {
        ...
        if (params != undefined) {
            for (let i = 0; i < params.length; i++) {
                if (params[i].width) {
                    item.style.width = params[i].width;
                }
                if (params[i].bGround) {
                ...
                if (params[i].iText) {
                    item.children[0].children[0].children[0].setAttribute('placeholder', params[i].iText);
                }
            }
        }
    }
...
function clickDontSelect(item, option) {
    document.onclick = function(e) {
        if (e.target != item && e.target != item.children[0] && e.target != item.children[1].children[0] && e.target != item.children[0].children[0]) {
